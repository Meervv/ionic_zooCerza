...
exports.create = async(req, res, next) => {
    try {
        const [rows] = await conn.execute('INSERT INTO animal SET nom=?, idEnclos=?, idEspece=?, idSante=?, image=?', [
            req.body.nom,
            req.body.idEnclos,
            req.body.idEspece,
            req.body.idSante,
...
exports.update = async(req, res, next) => {
    try {
        const [rows] = await conn.execute('UPDATE `animal` SET nom=?, idEnclos=?, idEspece=?, idSante=?, image=? WHERE id=?', [
            req.body.libelle,
            req.params.id
        ]);
        if (rows.affectedRows === 1) {
